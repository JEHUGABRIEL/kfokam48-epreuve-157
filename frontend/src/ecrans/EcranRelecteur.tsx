 ...
 *
 * L'identification reste visible : un relecteur est un étudiant assigné à ce rôle sur un exercice
 * donné (RG5), et rien ne dirait, si la liste disparaissait, sous quel nom les relectures affichées
 * ont été lues.
 */
...
export function EcranRelecteur({ section }: Props) {
          ...
          {assignees !== null &&
            assignees.map((relecture) => (
              <div className="carte interne" key={relecture.id}>
                <p>
                  Relecture n° <strong>{relecture.id}</strong> — exercice {relecture.exerciceId},
                  session {relecture.sessionId}
                </p>
                <p>
